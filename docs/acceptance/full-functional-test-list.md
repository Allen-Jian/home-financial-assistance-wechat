...
|---|---|---|---|---|
| AUTO-01 | 本地自动化测试 | Jest 全量 suites/tests 通过 | `npm test -- --runInBand`：29 suites、212 tests 通过 | 通过 |
...
| AUTO-06 | 异步写入竞态与重试安全 | AI 发送/清除、图片草稿绑定/确认、原件上传失败重试均不越过人工确认边界 | `tests/ai-page.test.ts`、`tests/photo-entry.test.ts`、`tests/imports-page.test.ts` deferred 回归覆盖；imports 本轮 21 tests 全绿 | 通过 |
