...
验收清单：`docs/acceptance/full-functional-test-list.md` 已新增 `2026-08-31 主题、AI 聊天、图片来源与自定义导航` 矩阵。自动化项目使用 `通过`；三主题重启、原生栏/回弹、当前 DevTools、真机和生产 API 按实际证据保持未执行或授权待执行。
...
- 生产 `/v1/ai/chat` 的 502/null-scope 验收保持 `未执行（待用户单独授权）`；继续操作前需要用户明确授权“仅重建并发布 VPS API 服务”。本报告不宣称生产 502 已解决。
